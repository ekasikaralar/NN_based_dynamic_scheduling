...
namespace simulation {
		...
		if (myfile_pol != -1){			
			...
			for (int iter = 0;  iter < num_iterations; iter++){
				...
				simulation::Execute exec(class_no, arr_cdf, lambda, mu_hourly, theta_hourly, no_server,
          									holding_cost_rate, abandonment_cost_rate, cost_rate, lambda_trans, num_interval, decision_freq, num_hidden_layers, activation_function, leaky_relu_slope, iter, scaling_factor); 
	...
	std::vector<std::vector<float>> MyNetwork::forward(const std::vector<std::vector<float>>& x) {
        ...
        return matmul(out, transpose(weights[num_hidden_layers])); // Final layer (no activation)
    }
	...
	std::vector<double> Execute::run(const std::vector<double>& initialization, const std::string& neural_network_folder_name)
	{	
		...
		for (int i = 0; i < network_size; i++) {
			nn_zs[i] = MyNetwork(num_hidden_layers, activation_function, leaky_relu_slope); // Initialize each object
		}
		...
		for (int i = 1; i <= end; i+= increment){
			...
			nn_zs[ind].load(neural_network_folder_name, file_name);
		}	
		
		//main part of the simulation
		while (sim_clock < T){
			//std::cout << " sim_clock " << sim_clock << std::endl;
...
int main(int argc, char** argv){ 
	...
	std::string jsonFileName = "/home/ekasikar/nn_simulations/config.json"; //the configuration file to initialize the simulation
	std::string record_file = "/home/ekasikar/nn_simulations/nn_policy.csv";  
