#include "ctmc_sim_2dim.h"

namespace simulation {

	Simulation::Simulation(const std::string& jsonFileName){

	class_no = 2; //this simulation is designed for 2-dimensional problems
	...
	// Close the JSON file
        ...
        optimal_policy_files = std::vector<std::vector<std::vector<double>>>(num_interval, std::vector<std::vector<double>>(upper[0], std::vector<double>(upper[1], 0)));
		...
		for (int i = start; i <= num_interval; i+= increment){
			...
			policy_file = policy_path + "policy" + std::to_string(i) + ".csv";
			int ind = (i - increment)/increment;
			optimal_policy_files[ind] = readMatrixFromCSV(policy_file);
		}
	...
	Execute::Execute(int& class_no_,
							...
							std::vector<std::vector<std::vector<double>>>& optimal_policy_files_,
	...
	void Execute::optimal_policy_calculation(std::vector<std::vector<double>>& optimal_policy_matrix){
			...
			if ((x1 >= upper[0]) || (x2 >= upper[1])){
				opt_policy = 0;
			} else {
			   opt_policy = optimal_policy_matrix[x1][x2]; //finding the optimal policy from the optimal policy matrix given the state
			}

			//in our matlab code, we use the following numbers to determine which policy is optimal:
			//if the optimal policy is 0: class 2 should be prioritized over class 1
			//if the optimal policy is 1: class 1 should be prioritized over class 2
			//if the optimal policy is 2: there are enough servers to serve all people in the system, so we set to default priority order
			if (opt_policy == 2){
				//priority order does not matter since we have enough servers to serve
				//set to default order
				priority_order = {1, 0}; //first class 2 is served then class 1 is served
			} else if (opt_policy == 1){
				//class 1 should be prioritized over class 2
				priority_order = {0, 1}; //first class 1 is served then class 2 is served
			} else if (opt_policy == 0){
				//class 2 should be prioritized over class 1
				priority_order = {1, 0};
			}
 	...
 	*/
	void Execute::handle_arrival_event(int& interval, int& cls, int& pre_interval, int& post_interval, std::vector<std::vector<double>>& optimal_policy_matrix){ 
	...
	/**
		...
		* @param optimal_policy_matrix The matrix that contains the optimal policy for a given state.
 	*/
	void Execute::handle_depart_event(int& interval, int& cls, int& pre_interval, int& post_interval, std::vector<std::vector<double>>& optimal_policy_matrix){ 
	...
	/**
		...
		* @param optimal_policy_matrix The matrix that contains the optimal policy for a given state.
 	*/
	void Execute::handle_abandon_event(int& interval, int& pre_interval, int& post_interval, std::vector<std::vector<double>>& optimal_policy_matrix){
	...
	{	
		...
		std::vector<std::vector<double>> optimal_policy_matrix(upper[0], std::vector<double>(upper[1], 0));
...
int main(int argc, char** argv){ 
	...
	std::string jsonFileName = "/home/ekasikar/ctmc_simulation/2dim/config_2dim/config_2dim.json"; //the configuration file to initialize the simulation
	std::string record_file = "/home/ekasikar/ctmc_simulation/2dim/2dim_ctmc_policy.csv";  
