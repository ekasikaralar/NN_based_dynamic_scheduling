...
namespace simulation {
	...
	void Execute::optimal_policy_calculation(std::vector<float>& optimal_policy_matrix){
			...
			if ((x1 > upper[0]) || (x2 > upper[1]) || (x3 > upper[2])){
				opt_policy = 3; //the default policy is serve class2, then class1 ,then class3
			} else {
			   opt_policy = optimal_policy_matrix[state];
			   // Fallback to default policy if the matrix provides an invalid policy
			   opt_policy = (opt_policy == 0) ? 3 : opt_policy;
			}
...
int main(int argc, char** argv){ 
	...
	std::string jsonFileName = "/home/ekasikar/ctmc_simulation/3dim/config_3dim/config_3dim.json"; //the configuration file to initialize the simulation
	std::string record_file = "/home/ekasikar/ctmc_simulation/3dim/3dim_ctmc_policy.csv";  
