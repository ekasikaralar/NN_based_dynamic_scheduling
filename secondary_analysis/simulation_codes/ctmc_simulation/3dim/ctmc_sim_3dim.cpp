#include "ctmc_sim_3dim.h"

namespace simulation {

	Simulation::Simulation(const std::string& jsonFileName){

		class_no = 3; //this simulation is designed for 3 dimensional problems
		...
		for (int i = start; i <= num_interval; i+= increment){
			...
			policy_file = policy_path + "policy" + std::to_string(i) + ".txt";
		...
		if (myfile_pol != -1){			
			...
			for (int iter = 0; iter < num_iterations; iter++){
				...
				simulation::Execute exec(class_no, arr_cdf, lambda, mu_hourly, theta_hourly, no_server, optimal_policy_files,
          									holding_cost_rate, abandonment_cost_rate, cost_rate, num_interval, decision_freq, increment, upper, iter); 
	...
	void Execute::optimal_policy_calculation(std::vector<float>& optimal_policy_matrix){
		std::vector<int> optimal_policy;
			...
			//find the state
			int x1 = num_in_system[1]; //number of people in class 1
			int x2 = num_in_system[2]; //number of people in class 2
			int x3 = num_in_system[3]; //number of people in class 3
			int opt_policy = 0;

			//calculating what state we are in -- in a sub2ind fashion
			unsigned long state = num_in_system[1] + upper[0] * num_in_system[2] + upper[0] * upper[1] * num_in_system[3];

			if ((x1 > upper[0] - 1) || (x2 > upper[1] - 1) || (x3 > upper[2] - 1)){
				opt_policy = 3; //the default policy is serve class2, then class1 ,then class3
			} else {
			...
			//if the optimal policy is 1: 1) class 1; 2) class 2; 3) class 3
			//if the optimal policy is 2: 1) class 1; 2) class 3; 3) class 2
			//if the optimal policy is 3: 1) class 2; 2) class 1; 3) class 3
			//if the optimal policy is 4: 1) class 2; 2) class 3; 3) class 1
			//if the optimal policy is 5: 1) class 3; 2) class 1; 3) class 2
			//if the optimal policy is 6: 1) class 3; 2) class 2; 3) class 1
			if (opt_policy == 1){
				priority_order = {0, 1, 2}; //first class1, then class2, then class3
			} else if (opt_policy == 2){
				priority_order = {0, 2, 1}; //first class1, then class3, then class2
			} else if (opt_policy == 3){
				priority_order = {1, 0, 2}; //first class2, then class1, then class3
			} else if (opt_policy == 4){
				priority_order = {1, 2, 0}; //first class2, then class3, then class1
			} else if (opt_policy == 5){
				priority_order = {2, 0, 1}; //first class3, then class1, then class2
			} else if (opt_policy == 6){
				priority_order = {2, 1, 0}; //first class3, then class2, then class1
			}
			...
			for (int i = 0; i < class_no; i++){
				...
				//num_in_service[0] += diff[i]; //changed these two
				num_in_service[i + 1] += diff[i];

				num_in_queue[0] -= diff[i];
				num_in_queue[i + 1] -= diff[i];

				//if the difference is positive, some people are removed from the queue and sent to service
				...
				//if the difference is negative, service for some people stops, and they are sent back to the queue to serve higher-priority classes
				else if (diff[i] < 0){
	...
	void Execute::handle_arrival_event(int& interval, int& cls, int& pre_interval, int& post_interval, std::vector<float>& optimal_policy_matrix){ 
	    ...
	    if ((num_in_system[0] <= no_server[interval] and pre_interval == post_interval) or ((num_in_service[0] < no_server[interval]) and pre_interval == post_interval)) {
			...
			if (num_in_service[0] < no_server[interval]) {
			    ...
			    remove_queue(cls); //removing the added job from the queue since the job has arrived in the service area
			    t_depart = sim_clock + generate_service(); //schedule service completion for the priority job
			
			//if there is no one in the system, then we make sure the next event is not a departure
			} else if (num_in_system[0] == 0){
			    t_depart = inf; 
			}
		}
		//if there are not enough servers to serve everyone in the system or if the number of agents has changed due to interval change (pre_interval != post_interval)
		else {
	...
	void Execute::handle_depart_event(int& interval, int& cls, int& pre_interval, int& post_interval, std::vector<float>& optimal_policy_matrix){ 
		...
		if (num_in_system[0] < no_server[interval] and pre_interval == post_interval){
			...
			if (num_in_system[0] > 0){
				t_depart = sim_clock + generate_service();
			} else {t_depart = inf;} //the next event has to be an arrival	
		}
...
int main(int argc, char** argv){ 
	...
	std::string jsonFileName = "/project/Call_Center_Control/analyses_final/ctmc_benchmarks/3dim/config_3dim.json"; //the configuration file to initialize the simulation
	std::string record_file = "/project/Call_Center_Control/analyses_final/ctmc_benchmarks/3dim/3dim_ctmc_policy.csv";  
